...
```
1. PM 打开 Studio → 看到左侧导航（产品组合 / 三个产品）与产品卡片网格
2. PM 点击产品卡片或侧边导航项 → 进入该产品的用户故事地图画布，了解其设计思路
...
左侧导航栏 + 内容区布局：产品组合页为卡片网格，画布页嵌入内容区。泳道横向排列（`SingleChildScrollView` + `Row`），每条泳道纵向堆叠任务卡片，故事卡片内嵌于任务卡片中。
