...
| 模块 | 职责 | 优先级 |
|------|------|--------|
| 用户故事地图画布 | 三层结构：用户活动分组 → 用户任务列 → 故事卡片 | P0 |
| MVP 发布线 | 可拖动的细浅色虚线，区分 MVP 与未来迭代 | P0 |
| 故事卡拖放 | 跨任务列拖放故事卡片，重新组织地图 | P0 |
...
```
1. PM 打开 Studio → 看到左侧导航（三个产品）与默认产品的故事地图画布（qtcloud-devops）
2. PM 点击侧边导航切换产品 → 看到该产品的用户故事地图画布，了解其设计思路
3. PM 在任务列间拖放故事卡片 → 重新组织用户故事
...
左侧导航栏 + 内容区布局：侧边导航选择产品，故事地图画布嵌入内容区。用户活动为上级分组自上而下排列（垂直滚动），组内用户任务为最小列横向排列（`SingleChildScrollView` + `Row`），列内纵向堆叠故事卡片。
...
| ActivitySection | 用户活动分组（包含任务列） |
| TaskColumn | 用户任务列（最小列）+ 拖放目标（DragTarget） |
| StoryCard | 故事明细卡（标题 + 描述 + 状态圆点，LongPressDraggable） |

### 3. 样式方案

- **主题**：Material 3，视觉减负——活动分组白底 + 细边框，任务列浅灰底，小色标仅作轻度区隔；故事卡片白色，状态用圆点（🟢 完成 / 🟡 进行中 / ⚪ 待办），MVP 发布线为细浅色虚线
- **实现**：Flutter Material Design

### 4. 交互

- 垂直滚动浏览活动分组，组内横向滚动浏览任务列
- 长按拖拽故事卡片跨任务列移动
- MVP 发布线拖拽调整版本范围
- 故事卡片点击交互（回调待实现业务逻辑）
